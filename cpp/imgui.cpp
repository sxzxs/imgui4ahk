...
bool ImGui::DockNodeCalcDropRectsAndTestMousePos(const ImRect& parent, ImGuiDir dir, ImRect& out_r, bool outer_docking, ImVec2* test_mouse_pos)
{
    ...
    else
    {
        hs_w = 16;
        hs_h = 16;
        off = ImVec2(ImFloor(hs_w * 2.25f), ImFloor(hs_w * 2.25f));
    }
...
static void ImGui::DockNodePreviewDockRender(ImGuiWindow* host_window, ImGuiDockNode* host_node, ImGuiWindow* root_payload, const ImGuiDockPreviewData* data)
{
    ...
    const ImU32 overlay_col_main = 0x99784f26;
    //const ImU32 overlay_col_main = GetColorU32(ImGuiCol_NavWindowingHighlight, is_transparent_payload ? 0.60f : 0.40f);

    const ImU32 overlay_col_bg = GetColorU32(ImGuiCol_DockingPreview_1, 1.f);
    const ImU32 overlay_col_drop = GetColorU32(ImGuiCol_DockingPreview, 1.f);
    const ImU32 overlay_col_out_line = GetColorU32(ImGuiCol_DockingOutLine, 1.f);
    const ImU32 overlay_col_lines = GetColorU32(ImGuiCol_DockingLine, 1.f);
    const ImU32 overlay_col_drop_hovered = overlay_col_lines;
    const ImU32 overlay_col_lines_hoverd = overlay_col_drop;
    ...
    if (data->IsDropAllowed && can_preview_tabs && data->SplitDir == ImGuiDir_None && data->IsCenterAvailable)
    {
        ...
        for (int payload_n = 0; payload_n < payload_count; payload_n++)
        {
            ...
            for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++)
            {
                ...
                TabItemLabelAndCloseButton(overlay_draw_lists[overlay_n], tab_bb, tab_flags, g.Style.FramePadding, payload->Name, 0, 0, true);
                if (!tab_bar_rect.Contains(tab_bb))
    ...
    // Display drop boxes
    const float overlay_rounding = 0.f;
    //const float overlay_rounding = ImMax(3.0f, g.Style.FrameRounding);
    //DockNodePreviewDockCalc
    if (data->IsCenterAvailable && data->IsSidesAvailable) {
        //overlay_col_bg
        ImVec2 center = data->DropRectsDraw[0].GetCenter();

        float max_x = center.x - data->DropRectsDraw[1].Min.x + 4;
        float corn = max_x * 0.55;
        ImVec2 points[] = {
            {-55 - 1, -20},
            {-31, -20},
            {-20, -31 + 1},
        ...
        int points_len = sizeof(points) / sizeof(ImVec2);
        for (int i = 0; i < points_len; i++)
            points[i] += center;
        for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++)
        {
            //overlay_draw_lists[overlay_n]->AddPolyline(points, points_len, overlay_col_out_line, false, 0.5f);
            //overlay

            overlay_draw_lists[overlay_n]->AddRectFilled(points[0], points[8], overlay_col_bg);
            overlay_draw_lists[overlay_n]->AddRectFilled(points[3], { points[4].x, points[0].y }, overlay_col_bg);
            overlay_draw_lists[overlay_n]->AddRectFilled({ points[12].x, points[15].y }, points[11], overlay_col_bg);

            overlay_draw_lists[overlay_n]->AddTriangleFilled(points[1], points[2], { points[2].x, points[1].y }, overlay_col_bg);
            overlay_draw_lists[overlay_n]->AddTriangleFilled(points[5], points[6], { points[5].x, points[6].y }, overlay_col_bg);
            overlay_draw_lists[overlay_n]->AddTriangleFilled(points[9], points[10], { points[10].x, points[9].y }, overlay_col_bg);
            overlay_draw_lists[overlay_n]->AddTriangleFilled(points[13], points[14], { points[13].x, points[14].y }, overlay_col_bg);
            //overlay_draw_lists[overlay_n]->AddRectFilled(points[1], { points[2].x, points[0].y }, overlay_col_bg);

            for (int i = 0; i < points_len - 1; i++) {
                ImVec2 offset = { 0,0 }, offset2 = { 0,0 };
                if (points[i + 1].x < points[i].x) offset.x = 1;
                //if (points[i + 1].y > points[i].y) offset.y = -1;
                if (i == 1) offset2.y = -1;
                if (i == 9) offset.y = -1;
                overlay_draw_lists[overlay_n]->AddLine(points[i] + offset, points[i + 1] + offset2, overlay_col_out_line, 1.f);
            }

            //overlay_draw_lists[overlay_n]->AddConvexPolyFilled(points, 10, 0xFFFF0000);
            //overlay_draw_lists[overlay_n]->AddConvexPolyFilled(points, points_len, 0xFFFF0000);
    ...
    for (int dir = ImGuiDir_None; dir < ImGuiDir_COUNT; dir++)
    {
        if (!data->DropRectsDraw[dir + 1].IsInverted())
        {
            ...
            bool is_wid_hovered = (data->SplitDir == (ImGuiDir)dir && data->IsSplitDirExplicit);
            ImU32 overlay_col = is_wid_hovered ? overlay_col_drop_hovered : overlay_col_drop;
            ImU32 overlay_col_line_ = is_wid_hovered ? overlay_col_lines_hoverd : overlay_col_lines;

            ImRect draw_r_in = draw_r;
            ImVec2 center = ImFloor(draw_r_in.GetCenter());
            draw_r_in.Expand(-4.0f);

            for (int overlay_n = 0; overlay_n < overlay_draw_lists_count; overlay_n++)
            {

                overlay_draw_lists[overlay_n]->AddRectFilled(draw_r.Min, draw_r.Max, overlay_col, 3);
                overlay_draw_lists[overlay_n]->AddRect(draw_r_in.Min, draw_r_in.Max, overlay_col_line_, overlay_rounding);
                overlay_draw_lists[overlay_n]->AddRectFilled(draw_r_in.Min, { draw_r_in.Max.x, draw_r_in.Min.y + 3 }, overlay_col_line_, overlay_rounding);
                if (dir == ImGuiDir_Left || dir == ImGuiDir_Right)
                    overlay_draw_lists[overlay_n]->AddLine(ImVec2(center.x, draw_r_in.Min.y), ImVec2(center.x, draw_r_in.Max.y), overlay_col_line_);
                if (dir == ImGuiDir_Up || dir == ImGuiDir_Down)
                    overlay_draw_lists[overlay_n]->AddLine(ImVec2(draw_r_in.Min.x, center.y + 1.f), ImVec2(draw_r_in.Max.x, center.y + 1.f), overlay_col_line_);
            }
        }
